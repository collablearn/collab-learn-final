...
import type { CreatedGuildReference, CreatedProjectReferece, JoinedGuildReference, UserReference } from "$lib/types";
...
export const load: LayoutServerLoad = async ({ locals: { supabase, safeGetSession } }) => {
    ...
    return {
        ...
        createdProjects: await supabase.from("created_projects_tb").select("*") as PostgrestSingleResponse<CreatedProjectReferece[]>
    }
